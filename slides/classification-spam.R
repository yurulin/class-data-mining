# classification-spam.R - Spam Email Classification
# @author: Yu-Ru Lin
# @date: 2014-01-30
...
data.url = 'http://www.yurulin.com/class/spring2014_datamining/data/ml_hackers/emaildata.zip'
...
# is SPAM, which we set to 0.5 (simple cutoff), and constant value for the
...
spam.df[1:3,1:3]
head(spam.df[with(spam.df, order(-spam.occurrence)),])  
...
head(easyham.df[with(easyham.df, order(-easyham.occurrence)),])

plot_two_terms_by_email_type <- function() {
  ...
  ggplot(init.df, aes(x = html, y = table)) +
    ...
    theme_bw()
  ggplot(init.df, aes(x = html, y = table)) +
    geom_point(aes(shape = type,color=type), position = "jitter") +
